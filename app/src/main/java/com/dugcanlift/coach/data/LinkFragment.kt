 ...
 * pasted twice back-to-back); otherwise the whole (trimmed) text is treated as the fragment. Either
 * way, the result is then cut at the first character outside the base64url alphabet
 * (`[A-Za-z0-9_-]`) -- matching iOS's decoder (`dugcanlift-kit ShareLink.swift:132`,
 * `rest.prefix { isBase64URLCharacter($0) }`), which discards everything from that point on
 * regardless of whether it was reached via a `#`-bearing URL or handed a bare fragment directly
 * (`decode(link:)`, `ShareLink.swift:169-174`, routes both cases into the same `decode(fragment:)`).
 * This is what SHARE-FORMAT.md's own email template needs: a mail client that wraps the link in
 * angle brackets (`<https://.../#1z...>`) or a sentence that ends with it (`...#1zAbC.`) leaves a
 * trailing character that is not part of the payload, and the same is true of a bare fragment
 * pasted with trailing punctuation or a caption -- a share-sheet caption trailing the link (e.g.
 * "...#XYZ thanks!") is cut at its leading space the same way a whitespace-only trim would have.
 * Either way the result is trimmed of surrounding whitespace first.
 */
fun fragmentFrom(text: String): String {
    val lastHash = text.lastIndexOf('#')
    val raw = if (lastHash < 0) text.trim() else text.substring(lastHash + 1).trim()
